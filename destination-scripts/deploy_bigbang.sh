...
deployFlux() {
/usr/local/bin/kubectl get ns flux-system || /usr/local/bin/kubectl create ns flux-system
/usr/local/bin/kubectl kustomize ${artifact_dir}/flux > ${artifact_dir}/flux/flux-deploy.yaml # transform flux manifest with edge settings
cp ${artifact_dir}/flux/flux-deploy.yaml ${k3s_root_dir}/server/manifests/flux-deploy.yaml
sleep 30
